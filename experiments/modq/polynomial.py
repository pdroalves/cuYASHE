...
class Polynomial():
    ...
    def __div__(self, b):
        ...
        if type(b) in (int,long,float):
            return (self // Polynomial(coef=[b],mod=self.mod))[0]
        else:
            return (self // b)[0]
    ...
    def __floordiv__(self,other):
        ...
        #     return Polynomial(coef=[c / other for c in self.coef],irr_poly=self.irr_poly),Polynomial(coef=[c % float(other) for c in self.coef],irr_poly=self.irr_poly)
        # if len(other.coef) == 1:
        #     return self // other.coef[0]
        ...
        # if divisor[-1] != 1: # Monic
        #     raise Exception("floordiv not implemented for non monic polynomials")
        # xi = 0
        # result = []
        # for i in xrange(-1, -len(dividend) - 1, -1):
        #     xi = dividend[i] - xi * divisor[0]
        #     result.insert(0, xi)
        # return Polynomial(coef=result[1:],irr_poly=self.irr_poly), Polynomial(coef=[result[0]],irr_poly=self.irr_poly)

        div = long(divisor.coef[-1])

        if len(dividend) >= len(divisor):
            #Shift divisor towards right so it's the same degree as dividend
            #shiftlen = len(dividend) - len(divisor)
            #divisor = [0] * shiftlen + divisor
            pass
        else:
